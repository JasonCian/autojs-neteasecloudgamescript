...
//循环管理
function seatcheck(){
    var 用户栏 = id("live_user_grid_container").className("android.widget.HorizontalScrollView").findOne(50);
    var userinfo = ["admin","none","none","none","none","none",]
    var olduserinfo = ["admin","none","none","none","none","none",]
    while(1){
    sleep(500)
    for (var i=1;i<6;i++){
        var 麦位状态 = 用户栏.child(0).child(i)
        sleep(100)
    ...
    if(olduserinfo[1] == userinfo[1] && olduserinfo[1] != '麦位无人'){
        ...
        if(olduserinfo[2] == true){
            ...
            if (controltime >= 30 && controltime <40){
                sendmsgtochat("@"+ userinfo[1] + " 已经使用控制权"+ controltime +"秒,请在3分钟之内归还控制权，否则将遭遇强制下机")
            }
            //if (controltime >= 900){
            if (controltime >= 30){
                getadmin(userinfo[5])
        ...
        if(olduserinfo[3] == true){
            ...
            log("申请时长:"+ applytime)
            if(checkadmin()){
                //if (applytime >= 720 && applytime <725){
